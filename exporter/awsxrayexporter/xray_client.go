...
const exporterPrefix = "xray-otel-exporter/"
...
// Get exact version of awsxrayexporter module for user-agent
...
func newXRay(logger *zap.Logger, awsConfig *aws.Config, buildInfo component.BuildInfo, s *session.Session) xrayClient {
	...
	x.Handlers.Build.PushBackNamed(request.NamedHandler{
		...
		Fn:   request.MakeAddToUserAgentFreeFormHandler(exporterPrefix + getModVersion() + execEnvPrefix + execEnv + osPrefix + osInformation),
	})
